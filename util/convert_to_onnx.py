    ...
    try:
        subprocess.run(
            ["python", "-m", "tf2onnx.convert", "--saved-model",
                str(saved_model_path), "--output", str(output_path)],
            check=True
        )
    ...
    try:
        subprocess.run(
            ["python", "-m", "tf2onnx.convert", "--tflite",
                str(tflite_model_path), "--output", str(output_path)],
            check=True
        )
...
def convert_keras_to_onnx(keras_model_path, output_path):
    try:
        subprocess.run(
            ["python", "-m", "tf2onnx.convert", "--keras",
                str(keras_model_path), "--output", str(output_path)],
            check=True
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert models to ONNX format.")
    parser.add_argument(
        "model_type", choices=["saved_model", "tflite", "keras"],
        help="Type of the model to convert (saved_model, tflite, keras)."
    )
    parser.add_argument(
        "input_path", help="Full path to the input model file."
    )
    parser.add_argument(
        "output_path", help="Full path to the output ONNX file."
    )
